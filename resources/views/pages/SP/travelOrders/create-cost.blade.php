...
@section('content')
    <div class="main-content-container overflow-hidden">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4 mt-1">
            <h3 class="mb-0">Tambah Biaya Perjalanan Dinas</h3>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb align-items-center mb-0 lh-1">
                    ...
                    <li aria-current="page" class="breadcrumb-item active"><span class="text-secondary">Tambah Biaya</span>
                    </li>
                </ol>
            </nav>
        </div>
        ...
        <form action="{{ route('sp.travelCosts.store') }}" method="POST" id="costForm">
            @csrf
            <input type="hidden" name="travel_order_id" value="{{ $travelOrder->id }}">
                        ...
                        <table class="table table-bordered align-middle" id="dailyTable">
                            ...
                            <tbody id="dailyBody">
                                ...
                                @empty
                                    <tr id="daily-row-0">
                                        <td>
                                            <input type="text" name="daily[0][employee_name]"
                                                class="form-control h-45" placeholder="Nama pegawai"
                                                value="{{ old('daily.0.employee_name') }}">
                                        </td>
                                        <td>
                                            <input type="number" name="daily[0][amount_per_day]"
                                                class="form-control h-45 amount-per-day" placeholder="0" min="0"
                                                step="1000" value="{{ old('daily.0.amount_per_day') }}"
                                                oninput="calcDaily(0)">
                                        </td>
                                        <td>
                                            <input type="number" name="daily[0][days]"
                                                class="form-control h-45 days-input" placeholder="0" min="1"
                                                value="{{ old('daily.0.days') }}" oninput="calcDaily(0)">
                                        </td>
                                        <td>
                                            <input type="number" name="daily[0][total_amount]"
                                                class="form-control h-45 daily-total bg-light" placeholder="0" readonly
                                                value="{{ old('daily.0.total_amount') }}">
                                        </td>
                                        <td class="text-center">
                                            <button type="button" class="btn btn-sm btn-danger p-1"
                                                onclick="removeRow('daily-row-0')" title="Hapus baris">
                                                <i class="ri-delete-bin-line fs-14"></i>
                                            </button>
                                        </td>
                                    </tr>
                            ...
                            <div class="form-group mb-4">
                                ...
                                <div class="form-group position-relative">
                                    <input type="number" name="pocket_money[amount]"
                                        ...
                                        step="1000" value="{{ old('pocket_money.amount', 0) }}">
                                    <i
                                    ...
                                    <td>
                                        <select name="accommodations[0][category_id]" class="form-select h-45">
                                            ...
                                            @foreach ($accommodationCategories as $cat)
                                                <option value="{{ $cat->id }}"
                                                    {{ old('accommodations.0.category_id') == $cat->id ? 'selected' : '' }}>
                                                    {{ $cat->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="transports[0][category_id]"
                                            ...
                                            @foreach ($transportCategories as $cat)
                                                <option value="{{ $cat->id }}" data-name="{{ $cat->name }}"
                                                    {{ old('transports.0.category_id') == $cat->id ? 'selected' : '' }}>
                                                    {{ $cat->name }}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="form-group mb-4">
                                ...
                                <div class="form-group position-relative">
                                    <input type="number" name="representative[amount]"
                                        class="form-control text-dark ps-5 h-55" placeholder="0" min="0"
                                        step="1000" value="{{ old('representative.amount', 0) }}">
                                    <i
                ...
                <div class="card-body p-20">
                    <div class="d-flex justify-content-end gap-3">
                        ...
                        <button type="submit" class="btn btn-primary py-3 px-5 fw-semibold text-white">
                            <i class="ri-save-line me-1"></i>Simpan Biaya
                        </button>
                    </div>
                </div>
            </div>
...
@push('scripts')
    <script>
        // Counter untuk baris dinamis — mulai dari jumlah pegawai yang sudah di-render
        let dailyCount = {{ $travelOrder->employees->count() ?: 1 }};
        let accCount = 1;
        ...
        function addDailyRow() {
            ...
            <tr id="daily-row-${idx}">
                <td>
                    <input type="text" name="daily[${idx}][employee_name]"
                        class="form-control h-45" placeholder="Nama pegawai">
                </td>
                <td>
                    <input type="number" name="daily[${idx}][amount_per_day]"
                        class="form-control h-45 amount-per-day"
                        placeholder="0" min="0" step="1000"
                        oninput="calcDaily(${idx})">
                </td>
                <td>
                    <input type="number" name="daily[${idx}][days]"
                        class="form-control h-45 days-input"
                        placeholder="0" min="1"
                        oninput="calcDaily(${idx})">
                </td>
                <td>
                    <input type="number" name="daily[${idx}][total_amount]"
                        class="form-control h-45 daily-total bg-light"
                        placeholder="0" readonly>
                </td>
                <td class="text-center">
                    <button type="button" class="btn btn-sm btn-danger p-1"
                        onclick="removeRow('daily-row-${idx}')">
                        <i class="ri-delete-bin-line fs-14"></i>
                    </button>
                </td>
            </tr>
        ...
        function addAccommodationRow() {
            ...
            <tr id="acc-row-${idx}">
                <td>
                    <select name="accommodations[${idx}][category_id]" class="form-select h-45">
                        <option value="">-- Pilih --</option>${options}
                    </select>
                </td>
                <td>
                    <input type="text" name="accommodations[${idx}][hotel_name]"
                        class="form-control h-45" placeholder="Nama hotel/penginapan">
                </td>
                <td>
                    <input type="number" name="accommodations[${idx}][price_per_night]"
                        class="form-control h-45 price-per-night"
                        placeholder="0" min="0" step="1000"
                        oninput="calcAccommodation(${idx})">
                </td>
                <td>
                    <input type="number" name="accommodations[${idx}][duration_nights]"
                        class="form-control h-45 duration-nights"
                        placeholder="0" min="1"
                        oninput="calcAccommodation(${idx})">
                </td>
                <td>
                    <input type="number" name="accommodations[${idx}][total_amount]"
                        class="form-control h-45 acc-total bg-light"
                        placeholder="0" readonly>
                </td>
                <td class="text-center">
                    <button type="button" class="btn btn-sm btn-danger p-1"
                        onclick="removeRow('acc-row-${idx}')">
                        <i class="ri-delete-bin-line fs-14"></i>
                    </button>
                </td>
            </tr>
        ...
        function addTransportRow() {
            ...
            <tr id="trans-row-${idx}">
                <td>
                    <select name="transports[${idx}][category_id]"
                        class="form-select h-45 transport-category"
                        onchange="toggleAirlineFields(${idx}, this.value)">
                        <option value="">-- Pilih --</option>${options}
                    </select>
                </td>
                <td>
                    <input type="number" name="transports[${idx}][amount]"
                        class="form-control h-45" placeholder="0" min="0" step="1000">
                </td>
                <td>
                    <input type="text" name="transports[${idx}][airline_name]"
                        class="form-control h-45 airline-field"
                        placeholder="Maskapai" style="opacity:0.4;" disabled>
                </td>
                <td>
                    <input type="text" name="transports[${idx}][booking_code]"
                        class="form-control h-45 booking-field"
                        placeholder="Kode booking" style="opacity:0.4;" disabled>
                </td>
                <td>
                    <input type="text" name="transports[${idx}][ticket_number]"
                        class="form-control h-45 ticket-field"
                        placeholder="No. tiket" style="opacity:0.4;" disabled>
                </td>
                <td>
                    <input type="text" name="transports[${idx}][note]"
                        class="form-control h-45" placeholder="Keterangan">
                </td>
                <td class="text-center">
                    <button type="button" class="btn btn-sm btn-danger p-1"
                        onclick="removeRow('trans-row-${idx}')">
                        <i class="ri-delete-bin-line fs-14"></i>
                    </button>
                </td>
            </tr>
