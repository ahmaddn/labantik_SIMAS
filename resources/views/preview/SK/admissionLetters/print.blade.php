...
<head>
    ...
    <title>Surat Keterangan Penerimaan Siswa - {{ $admission->letter_number }}</title>
...
<body>
    <div class="action-buttons">
        <div style="display: flex; gap: 10px; align-items: center;">
            ...
            <a href="{{ route('sk.admissionLetters.index') }}" class="btn btn-back">
                <i class="fa-solid fa-arrow-left"></i> Kembali
            </a>
        </div>

        <div style="display: flex; gap: 10px">
            <a href="{{ route('sk.admissionLetters.edit', $admission->id) }}" class="btn btn-edit">
    ...
    <div class="page clearfix">
        ...
        <div class="judul-surat">
            ...
            <p>Nomor : {{ $admission->letter_number }}</p>
        </div>

        <!-- ISI SURAT -->
        <div class="isi-surat">
            <p>
                Yang bertanda tangan di bawah ini, Kepala SMK Negeri 1 Talaga menerangkan dengan sebenarnya bahwa :
            </p>
            ...
            <table class="tabel-data">
                <tr>
                    ...
                    <td class="col-sep">:</td>
                    <td class="col-value bold-text">{{ $admission->student->student->full_name ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="col-label">NISN</td>
                    <td class="col-sep">:</td>
                    <td class="col-value">{{ $admission->student->student->national_student_number ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="col-label">Nama Orang Tua</td>
                    <td class="col-sep">:</td>
                    <td class="col-value">{{ $admission->student->student->parent_name ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="col-label">Asal Sekolah</td>
                    <td class="col-sep">:</td>
                    <td class="col-value">{{ $admission->previous_school ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="col-label">Alamat</td>
                    <td class="col-sep">:</td>
                    <td class="col-value">{{ $admission->student->student->address ?? '-' }}</td>
                </tr>
            </table>
    ...
    <script>
        function handlePrint() {
            ...
            fetch("{{ route('sk.admissionLetters.increment-download', $admission->id) }}", {
                    method: 'POST',
                    headers: {
